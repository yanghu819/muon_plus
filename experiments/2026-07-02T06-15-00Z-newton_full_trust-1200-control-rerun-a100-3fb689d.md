...
Step 600 keeps Lite256 ahead, but the gap is narrow. Step 300 reproduced full
...
the 300 gate). Step 600 is also behind Lite256 (`3.9177` vs `3.9162`), but only
by `0.0015`. Continue to 900/1200. If final full Trust is `<= 3.6895`, stop the
temporal schedule. If full Trust is worse than `3.6895`, promote one full-length
Lite256->full Trust run, while noting that the rerun used the restarted GPU2
A100 instance.
